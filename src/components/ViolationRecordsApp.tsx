...
function displayLogDescription(description: string) {
  return description.replaceAll("(null)", "(No Student Number)");
}
...
function LogsPage({ logs, filters, setFilters, applyFilters, resetFilters }: { logs: LogEntry[]; filters: Record<string, string>; setFilters: (filters: Record<string, string>) => void; applyFilters: () => void; resetFilters: () => void }) {
  ...
  const normalizedLogs = logs.map((log) => ({ ...log, description: displayLogDescription(log.description) }));
  const pager = usePaged(normalizedLogs);
  return (
